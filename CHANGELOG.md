...
**버전**: 폴더·문서는 v1~v8. 계산기 화면 표기만 v4→Mark3, v5→Mark4, v6→Mark5, v7→Mark7, v8→Mark8로 표시.
...
- **2번 항목(K값 선택)** 제거, K값을 `1`로 고정
- **3번 항목** 입력 박스 하단 가이드 문구 추가: "실험 대상 지면의 일평균 방문 수를 입력해주세요."
- **7번 항목** 가이드 문구 변경: "목표하는 지표의 최근 일평균 값을 입력해주세요. (CTR, CVR 등)"

### Technical
- `getKValueOrThrow()`를 고정값 반환 구조(`k=1`)로 단순화
- `visit-frequency`, `custom-k` 관련 이벤트/요약 텍스트 분기 제거
...
| 버전 | UI 표기 | 출시일 | 주요 기능 |
|------|--------|--------|-----------|
| v8 | Mark8 | 2026-02-23 | K값 고정(1), 2번 항목 제거, 3번/7번 가이드 문구 개선 |
