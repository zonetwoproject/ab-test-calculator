...
const WEBAPP_PROD_URLS = [
  'https://ab-test-calculator-one.vercel.app/versions/v9/index.html',
  'https://ab-test-calculator-one.vercel.app/versions/v9/',
  'https://ab-test-calculator-one.vercel.app/versions/v9',
  'https://ab-test-calculator-one.vercel.app/',
];
  ...
  (function () {
    function applyInAppStyle() {
      ...
      style.textContent = [
        ...
        '.content[data-active-tab=\"guide\"] .calculator-pane { display: block !important; }'
      ].join(' ');
...
function getWebAppUrls() {
  ...
  // Keep production first for release, but always include localhost fallback.
  if (!__DEV__) return [...WEBAPP_PROD_URLS, webLocal, previewLocal];

  // In debug, prioritize local servers for fast iteration.
...
const WebAppScreen = React.forwardRef<WebAppScreenHandle>(function WebAppScreen(_, ref) {
